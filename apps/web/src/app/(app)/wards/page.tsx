...
import { NewWardDialog } from "@/components/wards/NewWardDialog";
...
export default function WardsPage() {
  ...
  return (
    <PageShell
      ...
      actions={canCreate ? <NewWardDialog /> : null}
    >
        ...
        {isLoading ? (
          ...
          wards?.map((ward) => (
            <Card key={ward.id} className="hover:border-primary transition-colors">
              ...
              <CardContent>
                {ward.specialty && (
                  <p className="text-sm text-muted-foreground">{ward.specialty}</p>
                )}
                {ward.floor && (
                  <CardDescription className="mt-2">Andar {ward.floor}</CardDescription>
                )}
